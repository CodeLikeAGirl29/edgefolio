...
module.exports = plugin.withOptions(
  (options = {}) => {
    return ({ addBase }) => {
      ...
      const stylesToAdd = Object.entries(themes).reduce(
        (cssClasses, [themeName, theme]) => {
          const styles = {};

          Object.entries(flattenColorPalette(theme)).forEach(
            ([color, hex]) =>
              hex &&
              (styles["--color-" + color] = Color(hex).rgb().array().join(" ")),
          );

          const cssSelector =
            themeName === "default" ? "html" : `.${themeName}`;
  ...
  function (options = {}) {
    ...
    const colors = Object.values(themes).reduce((variables, theme) => {
      Object.keys(flattenColorPalette(theme)).forEach(
        (rule) =>
          (variables[rule] = `rgb(var(--color-${rule}) / <alpha-value>)`),
      );
      return variables;
    }, {});
